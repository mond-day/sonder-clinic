...
- **Google personal pull (A38):** pull-sync + overlay; oauth-status expõe `redirectUri` + `mock`/`mockEnvPresent`; Client ID/Secret na UI (1.3.5+); parse MOCK robusto + diagnóstico (1.3.6); fail-fast MOCK só se `true` (1.3.7)
- **Produção MOCK:** `stack.production.yml` defaulta `GOOGLE_CALENDAR_MOCK=false` e `NIBO_MOCK=false` (api/worker); fail-fast + `deploy.sh` se MOCK **ligado**; ausência em prod = off (1.3.7+); `NIBO_PULL_*` injetados
