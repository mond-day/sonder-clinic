...
- **Google personal pull (A38):** pull-sync + overlay; oauth-status expõe `redirectUri` + `mock`/`mockEnvPresent`; Client ID/Secret na UI (1.3.5+); parse MOCK robusto + diagnóstico (1.3.6)
- **Produção MOCK:** `stack.production.yml` defaulta `GOOGLE_CALENDAR_MOCK=false` e `NIBO_MOCK=false` (api/worker); fail-fast + `deploy.sh` (agora faz `source .env`) se MOCK ligado; `NIBO_PULL_*` injetados
