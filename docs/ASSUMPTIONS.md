...
| A38 | Google Calendar OAuth | **Fatia 4 + remessa P2 + gap 2026-09 + 1.3.6:** OAuth real estilo N8N — Client ID/Secret na UI (conexão criptografada), env `GOOGLE_CLIENT_*` opcional como fallback. Redirect canônico `…/api/v1/integrations/google/callback` (env `GOOGLE_REDIRECT_URI` ou derivado de `API_URL`). Sync clinic→Google (outbox); Google→clinic via pull-sync **e** webhook se `GOOGLE_CALENDAR_WEBHOOK_URL`. Pull-sync conta eventos pessoais (overlay) e falhas (MOCK/OAuth) na API/UI. Auto-renew no worker se `GOOGLE_CALENDAR_WATCH_AUTO_RENEW=true`. Exige `GOOGLE_CALENDAR_MOCK=false` + OAuth — **GO só com OAuth concluído**. Parse de MOCK: só `true/1/yes` liga; `false/0/no` desliga; ausência = MOCK implícito. oauth-status expõe `mock` + `mockEnvPresent`. Swarm defaulta MOCK=false; fail-fast + `deploy.sh` (carrega `.env`) recusam MOCK ligado |
