...
| A38 | Google Calendar OAuth | **Fatia 4 + remessa P2 + gap 2026-09 + 1.3.4:** OAuth real; sync clinic→Google (outbox); Google→clinic via pull-sync **e** webhook se `GOOGLE_CALENDAR_WEBHOOK_URL`. Pull-sync também conta eventos pessoais (overlay na Agenda com toggle “Eventos do Google”) e falhas (MOCK/OAuth/calendarId) são surfadas na API/UI (inclui `redirectUri` em oauth-status). Auto-renew no worker se `GOOGLE_CALENDAR_WATCH_AUTO_RENEW=true`. Sem webhook URL → pull-sync só. Exige `GOOGLE_CALENDAR_MOCK=false` + OAuth — **GO só com OAuth concluído**. Em produção o Swarm defaulta `GOOGLE_CALENDAR_MOCK=false`; fail-fast + `deploy.sh` recusam MOCK=true; redirect exato `https://<API_HOST>/api/v1/integrations/google/callback` |
