...
| A38 | Google Calendar OAuth | **Fatia 4 + remessa P2 + gap 2026-09:** OAuth real; sync clinic→Google (outbox); Google→clinic via pull-sync **e** webhook se `GOOGLE_CALENDAR_WEBHOOK_URL`. Pull-sync também conta eventos pessoais (overlay na Agenda com toggle “Eventos do Google”) e falhas (MOCK/OAuth/calendarId) são surfadas na API/UI. Auto-renew no worker se `GOOGLE_CALENDAR_WATCH_AUTO_RENEW=true`. Sem webhook URL → pull-sync só. Exige `GOOGLE_CALENDAR_MOCK=false` + OAuth — **GO só com OAuth concluído**. Em produção o Swarm defaulta `GOOGLE_CALENDAR_MOCK=false`; fail-fast recusa startup se MOCK=true/ausente |
...
| A50 | Nibo ↔ Financeiro | **Pull:** import manual `POST /integrations/:id/nibo/import` **e** pull periódico do worker (`finance.nibo-pull.requested`, intervalo `NIBO_PULL_INTERVAL_MS`, default 30min) — create **e update** por `externalId` (cancelados locais preservados). **Push:** outbox `finance.nibo-sync.requested` em create/update Receivable, create Payable, baixa (PAY) Receivable/Payable e cancel Payable (DELETE schedule). Worker: schedule credit/debit + receipts/payments (exige `accountId` na config) + DELETE. Sem estorno de baixa no Nibo. Produção: `NIBO_MOCK=false` (fail-fast) |
