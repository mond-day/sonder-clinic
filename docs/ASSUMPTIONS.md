...
| A49 | Release / prod bootstrap | Tag `vX.Y.Z` dispara CI + imagens + deploy Swarm (`migrate deploy`). Setup inicial em `/setup` (operador informa o token; API exige `X-Setup-Token`). API recusa startup inválido; worker também. Swagger off por default em prod. Readiness em `/api/v1/health/ready` (Traefik). `WEB_URL` canônico (HTTPS). Imagens do stack sem fallback `1.0.0`. |
