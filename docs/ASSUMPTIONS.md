...
| A50 | Nibo ↔ Financeiro | **Pull:** import manual `POST /integrations/nibo/:id/import` (alias legado `POST /integrations/:id/nibo/import`) **e** pull periódico do worker (`finance.nibo-pull.requested`, intervalo `NIBO_PULL_INTERVAL_MS`, default 30min) — create **e update** por `externalId` (cancelados locais preservados). Categorias filtram crédito **e** débito; centros de custo filtram só débito. IDs comparados case-insensitive. Worker loga tick/skip/complete (`skipReason` se MOCK/sem ACTIVE). **Push:** outbox `finance.nibo-sync.requested` em create/update Receivable, create Payable, baixa (PAY) Receivable/Payable e cancel Payable (DELETE schedule). Worker: schedule credit/debit + receipts/payments (exige `accountId` na config) + DELETE. Sem estorno de baixa no Nibo. Produção: `NIBO_MOCK=false` (fail-fast + deploy.sh) |
...
Última atualização: **Sync Nibo bidirecional create/update + pull automático (2026-09)**.
