...
| A49 | Release / prod bootstrap | Tag `vX.Y.Z` dispara CI + imagens + deploy Swarm. **1.3.9+:** `prisma migrate deploy` no boot de **api e worker** (`runBootMigrations`, advisory lock, fail-fast); serviço Swarm `migrate` opcional (CREATE DATABASE + pré-deploy). Setup inicial em `/setup` (operador informa o token; API exige `X-Setup-Token`). API/worker recusam startup inválido. Swagger off por default em prod. Readiness em `/api/v1/health/ready` (Traefik). `WEB_URL` canônico (HTTPS). Imagens do stack sem fallback `1.0.0`. |
